...
def find_p1_p2_p3_short(highs, lows):
    if not highs or not lows:
        return None, None, None
    p2 = lows[-1]
    p3_candidates = [h for h in highs if h[0] < p2[0]]
    if not p3_candidates:
        return p2, None, None
    p3 = p3_candidates[-1]
    p1_candidates = [l for l in lows if l[0] < p3[0]]
    if not p1_candidates:
        return p2, p3, None
    p1 = p1_candidates[-1]
    return p2, p3, p1
...
def analyze_long(ticker, grosswetterlage):
    ...
    if vol < MIN_AVG_VOLUME:
        r["reason"] = f"Volumen zu gering ({int(vol/1000)}k)"; return r
    df = get_60min_data(ticker)
    if df is None:
        r["reason"] = "Keine Daten"; return r
    ...
    p2_hoch = p2[1]; p3_tief = p3[1]; p1_hoch = p1[1]
    r["p1"] = round(p1_hoch,2); r["p2"] = round(p2_hoch,2); r["p3"] = round(p3_tief,2)
    try:
        current_close = float(df["Close"].iloc[-1])
    except:
        r["reason"] = "Kurs-Fehler"; return r
    if current_close <= p2_hoch:
        r["reason"] = f"Kurs {current_close:.2f} nicht über P2 {p2_hoch:.2f}"; return r
    entry = current_close
    p2_p3_distanz = p2_hoch - p3_tief
    max_entry = p2_hoch + p2_p3_distanz / 3
    if entry > max_entry:
        r["reason"] = f"1/3-Regel verletzt — Kurs {entry:.2f} > Max {max_entry:.2f}"; return r
    stop = p3_tief - 0.20
    if stop <= 0 or stop >= entry:
        r["reason"] = "Stop ungültig"; return r
    dist = (entry - stop) / entry
    r["stop_distanz"] = round(dist * 100, 1)
    if dist > MAX_STOP_DISTANCE:
        r["reason"] = f"Stop-Distanz {dist*100:.1f}% > {MAX_STOP_DISTANCE*100:.0f}%"; return r
    ziel = p3_tief + (p2_hoch - p1_hoch)
    risk = entry - stop
    crv  = (ziel - entry) / risk
    if crv < MIN_CRV:
        r["reason"] = f"CRV 1:{crv:.1f} unter Minimum 1:{MIN_CRV:.0f}"; return r
    ...
    r.update(signal=True, reason="✓ Alle Bedingungen erfüllt",
             ...
             crv=round(crv,1), position_stueck=round(stueck,0), position_wert=round(wert,0))
    return r
...
def analyze_short(ticker, grosswetterlage):
    ...
    if p3 is None: r["reason"] = "Kein P3 erkennbar"; return r
    if p1 is None: r["reason"] = "Kein P1 erkennbar"; return r
    p2_tief = p2[1]; p3_hoch = p3[1]; p1_tief = p1[1]
    r["p1"] = round(p1_tief,2); r["p2"] = round(p2_tief,2); r["p3"] = round(p3_hoch,2)
    try:
        current_close = float(df["Close"].iloc[-1])
    except:
        r["reason"] = "Kurs-Fehler"; return r
    if current_close >= p2_tief:
        r["reason"] = f"Kurs {current_close:.2f} nicht unter P2 {p2_tief:.2f}"; return r
    entry = current_close
    p2_p3_distanz = p3_hoch - p2_tief
    min_entry = p2_tief - p2_p3_distanz / 3
    if entry < min_entry:
        r["reason"] = f"1/3-Regel verletzt — Kurs {entry:.2f} < Min {min_entry:.2f}"; return r
    stop = p3_hoch + 0.20
    if stop <= entry:
        r["reason"] = "Stop ungültig"; return r
...
def build_dashboard(results, gw, dax_kurs, scan_time, modus):
    ...
    def crow(r):
        ...
        warnung = '<span style="color:#FF8F00;margin-right:6px" title="Gegen Großwetterlage">⚠️</span>' if r.get("warnung") else ""
        return f"""<tr>
          <td>{warnung}<strong>{r["name"]}</strong><br><small style="color:#666">{r["ticker"]}</small></td>
          <td class="num">{r["p1"]}</td><td class="num">{r["p2"]}</td><td class="num">{r["p3"]}</td>
    ...
    if any(r.get("warnung") for r in candidates):
        warnung_hinweis = f'<div style="background:#FF8F0018;border:1px solid #FF8F0055;border-radius:8px;padding:12px 16px;margin-bottom:20px;font-size:13px;color:#FF8F00">⚠️ &nbsp;Einige Kandidaten laufen <strong>gegen die Großwetterlage</strong> — erhöhte Anforderung an die Struktur. Eigenes Urteil erforderlich.</div>'
...
.badge{{display:inline-block;padding:5px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid {gw_color}55;background:{gw_color}18;color:{gw_color};margin-bottom:12px}}
.modus{{display:inline-block;margin-left:10px;padding:5px 14px;border-radius:20px;font-size:13px;font-weight:600;border:1px solid {farbe}55;background:{farbe}18;color:{farbe};margin-bottom:12px}}
...
<div class="stats">
  <div class="stat"><div class="n" style="color:{farbe}">{len(candidates)}</div><div class="l">Kandidaten</div></div>
...
def main():
    ...
    for c in candidates:
        ...
        print(f"  {warn}{c['name']:<8}  P1 {c['p1']:>8}  P2 {c['p2']:>8}  P3 {c['p3']:>8}  "
